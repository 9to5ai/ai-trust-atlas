...
export function readView(url: URL, maxYear: number): AtlasView {
  ...
  return {...(p.get('useCases')==='1'?{useCases:true}:{}),...(p.get('incidents')==='1'||selected?.startsWith('incident:')?{incidents:true}:{}),selected,layout,projection:routed??(p.get('view')==='use-cases'?'use-cases':p.get('view')==='questions'?'questions':p.get('view')==='list'?'list':p.get('view')==='focus'&&selected?'focus':'atlas'),query:(p.get('q')??'').slice(0,300),authorities:authorityOrder.filter(x=>p.getAll('type').includes(x)),regions:regionOrder.filter(x=>p.getAll('region').includes(x)),year:year>=1900&&year<=maxYear?year:maxYear,anchor:objectById.has(p.get('anchor')??'')?p.get('anchor')!:selected}
}
