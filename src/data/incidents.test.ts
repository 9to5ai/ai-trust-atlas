...
describe('incident evidence and integration',()=>{
 it('has only the approved seed and resolvable interpretive connections',()=>{
  expect(incidents.map(i=>i.id)).toEqual(['hugging-face-2026'])
 ...
 it('restores sourced incident questions for all roles and dates by findings, not ingestion',()=>{
  for(const role of ['board','executive','regulator'] as const){const q=incidentQuestion(incidents[0],role);expect(catalogue(role).some(e=>e.question.id===q.id)).toBe(true);expect(readBrief(JSON.stringify({version:1,ids:[q.id]})).selected).toEqual([q])}
  ...
  expect(incidentsForNode('concept','agent-authority')).toHaveLength(1)
 })
})
