...
export function IncidentDetail({item,onSelect}:{item:Incident;onSelect:(id:string)=>void}) {
 ...
 return <div className="incident-detail">
  <span className="inspector-kicker">Incident · {item.classification}</span><h2>{item.title}</h2>
  <p>{item.summary}</p><div className="incident-dates"><span>Occurred {item.occurred}</span><span>{item.disclosureLabel} {item.disclosed}</span><span>Findings published {item.updated}</span></div>
  ...
  <details className="inspector-section"><summary>Sources and limits</summary><p>{item.limitations}</p>{item.sources.map((s,n)=><p key={s.url}><a href={s.url} target="_blank" rel="noreferrer">[{n+1}] {s.title} ↗</a><small className="incident-source-role">{s.role}</small></p>)}<p>Reviewed {item.reviewed}. {item.reviewScope}</p></details>
 </div>
}
