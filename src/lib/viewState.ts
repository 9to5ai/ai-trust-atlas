...
export type AtlasView = { selected?: string; layout: LayoutMode; projection: 'atlas' | 'list' | 'focus'; query: string; authorities: AuthorityClass[]; regions: Instrument['region'][]; year: number; anchor?: string }
export function readView(url: URL, maxYear: number): AtlasView {
  ...
  return {selected,layout,projection:p.get('view')==='list'?'list':p.get('view')==='focus'&&selected?'focus':'atlas',query:(p.get('q')??'').slice(0,300),authorities:authorityOrder.filter(x=>p.getAll('type').includes(x)),regions:regionOrder.filter(x=>p.getAll('region').includes(x)),year:year>=1900&&year<=maxYear?year:maxYear,anchor:objectById.has(p.get('anchor')??'')?p.get('anchor')!:selected}
}
