...
import { audiences } from '../src/data/leadershipQuestions'
const issues = questionCoverageIssues()
if (issues.length) {
  console.error(`Question coverage failed:\n${issues.join('\n')}`)
  process.exitCode = 1
} else {
  console.log(`Question coverage passed: ${questionNodes.length} Atlas cards and ${developments.length} developments; all ${audiences.length} audiences.`)
}
